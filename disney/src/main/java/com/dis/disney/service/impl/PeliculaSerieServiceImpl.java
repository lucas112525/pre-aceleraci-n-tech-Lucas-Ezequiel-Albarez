...
public class PeliculaSerieServiceImpl implements PeliculaSerieService{
	...
	public void deletPeliculaSerie(int id) {
		...
		for(Personaje p:peliculaSerieRepository.getById((long)id).getPersonajes()) {
			p.deletPeliculaSerie(peliculaSerieRepository.getById((long)id));
		}
		for(Genero g:peliculaSerieRepository.getById((long)id).getGeneros()) {
			g.deletPeliculaSerie(peliculaSerieRepository.getById((long)id));
		}
